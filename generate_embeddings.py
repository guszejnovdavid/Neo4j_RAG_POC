...
def round_to_significant_digits(x, p, minval=1e-8):  # from Stackoverflow, modified
    ...
    if p is None:
        ...
        x_positive = np.where(
            np.isfinite(x) & (np.abs(x) > minval), np.abs(x), 10 ** (p - 1)
        )
        ...
        with open(target_file_path, "w", encoding="utf-8") as target_file:
            ...
            for row in reader:
                if len(row) and len(row[0]):
                    ...
                    processed_size = processed_size + sys.getsizeof(
                        ",".join(row)
                    )  # due to how getsizeof is implemented we can't just use getsizeof(row)
                    if len(read_rows) >= read_batch_size:
                        embeddings = embedding_model.client.encode(
                            batch, **embedding_model.encode_kwargs
                        )
                        ...
                        writer.writerows(
                            [
                                old_row + [convert_vector_to_neo4j_format(embedding)]
                                for old_row, embedding in zip(read_rows, embeddings)
                            ]
                        )  # save to new csv
